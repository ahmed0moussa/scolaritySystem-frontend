...
export class ListjsComponent {
  ...
  constructor(public service: ListService, private formBuilder: UntypedFormBuilder, private pipe: DecimalPipe,private classeService:ClasseService,private toastr:ToastrService) {
  ...
  saveListJs() {
    ...
    if (this.listJsForm.valid) {
      ...
      if (formData.ids) {
        ...
        this.classeService.updateClasse(formData.ids, formData).subscribe(
          () => {
        ...
        // Add new data
        this.classeService.createClasse(formData).subscribe(
          (response) => {
