...
export class ListjsComponent {
  ...
  constructor(public service: ListService, private formBuilder: UntypedFormBuilder, private pipe: DecimalPipe,private salleService:SalleService,private toastr:ToastrService) {
...
this.findall();
  ...
  saveListJs() {
    ...
    if (this.listJsForm.valid) {
      ...
      if (formData.ids) {
        ...
        this.salleService.updateSalle(formData.ids, formData).subscribe(
          () => {
            console.log('salle updated successfully', formData);
            this.toastr.success('salle modifié avec succès!', 'Success');
            this.findall();
          },
          (error) => {
    ...
    if (id) {
      ...
      this.salleService.deleteSalle(id).subscribe(
        () => {
          ...
          this.toastr.success('salle deleted successfully', 'Success');
          this.findall();
        },
        (error) => {
  ...
  */
  editModal(id: any) {
    ...
    this.listJsForm.controls['nom'].setValue(listData.firstName);
    this.listJsForm.controls['nmbrPlace'].setValue(listData.lastName);
  ...
  sortname() {
    this.ListJsDatas.sort(function (a: any, b: any) {
      ...
      if (a.nmbrPlace > b.nmbrPlace) { return 1; }
      return 0;
    })
  }
